...
RUN R -e "install.packages('pbdMPI', configure.args='--with-mpi-type=OPENMPI', repos='https://cloud.r-project.org/')"
...
ENV PKG_CPPFLAGS="-I/usr/local/include -I/usr/local/include/tbb -I/usr/local/cuda/include -I/usr/lib/aarch64-linux-gnu/openmpi/include"
ENV PKG_CXXFLAGS="-I/usr/local/include -I/usr/local/include/tbb -I/usr/local/cuda/include -I/usr/lib/aarch64-linux-gnu/openmpi/include"
ENV PKG_LIBS="-L/usr/local/lib -ltbb -L/usr/lib/aarch64-linux-gnu/openmpi/lib -lmpi"

RUN echo "Searching for mpi.h..." && find /usr/include /usr/lib /opt /usr/local -name mpi.h || true
