...
__global__ void sumAndBroadcast_kernel_optimized(
    const float* bVec,
    float* resultVec,
    ...
    int start = start_indices[group_idx];
    int end = start_indices[group_idx + 1];
    int group_size = end - start;

    int tid = threadIdx.x;
    int blockSize = blockDim.x;

    // Phase 1: Parallel reduction to compute sum
    float thread_sum = 0.0f;

    // Each thread sums multiple elements if group_size > blockSize
    ...
    // Thread 0 now has the complete sum
    float group_sum = sdata[0];
    __syncthreads();

    // Phase 2: Broadcast - all threads write the sum to their positions
    for (int i = start + tid; i < end; i += blockSize) {
        resultVec[i] = group_sum;
    }
}
...
int gpuSymMatMult_prodImatImatt(
    ...
    // Allocate GPU memory
    float* d_bVec = nullptr;
    float* d_result = nullptr;
    int* d_start_indices = nullptr;

    cudaStat = cudaMalloc((void**)&d_bVec, n * sizeof(float));
    if (cudaStat != cudaSuccess) {
        std::cerr << "[" << rank << "] "
                  << "Error: cudaMalloc() for d_bVec failed: " << cudaStat << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (cudaStat != cudaSuccess) {
        std::cerr << "[" << rank << "] "
                  << "Error: cudaMalloc() for d_result failed: " << cudaStat << std::endl;
        cudaFree(d_bVec);
        return EXIT_FAILURE;
    }

    cudaStat = cudaMalloc((void**)&d_start_indices, (n_groups + 1) * sizeof(int));
    if (cudaStat != cudaSuccess) {
        std::cerr << "[" << rank << "] "
                  << "Error: cudaMalloc() for d_start_indices failed: " << cudaStat << std::endl;
        cudaFree(d_bVec);
        cudaFree(d_result);
        return EXIT_FAILURE;
    }
    ...
    // Copy to GPU
    cudaStat = cudaMemcpy(d_bVec, bVec, n * sizeof(float), cudaMemcpyHostToDevice);
    if (cudaStat != cudaSuccess) {
        std::cerr << "[" << rank << "] "
                  << "Error: cudaMemcpy (host to device) for bVec failed: " << cudaStat << std::endl;
        cudaFree(d_bVec);
        cudaFree(d_result);
    ...
    cudaStat = cudaMemcpy(d_start_indices, start_indices,
                         (n_groups + 1) * sizeof(int), cudaMemcpyHostToDevice);
    if (cudaStat != cudaSuccess) {
        std::cerr << "[" << rank << "] "
                  << "Error: cudaMemcpy (host to device) for start_indices failed: " << cudaStat << std::endl;
        cudaFree(d_bVec);
        cudaFree(d_result);
        cudaFree(d_start_indices);
        return EXIT_FAILURE;
    ...
    sumAndBroadcast_kernel_optimized<<<blocks, threadsPerBlock, sharedMemSize>>>(
        d_bVec, d_result, d_start_indices, n_groups
    );

    cudaStat = cudaDeviceSynchronize();
    if (cudaStat != cudaSuccess) {
        std::cerr << "[" << rank << "] "
                  << "Error: kernel execution failed: " << cudaStat << std::endl;
        cudaFree(d_bVec);
        cudaFree(d_result);
        cudaFree(d_start_indices);
        return EXIT_FAILURE;
    ...
    // Copy result back
    cudaStat = cudaMemcpy(resultVec, d_result, n * sizeof(float), cudaMemcpyDeviceToHost);
    if (cudaStat != cudaSuccess) {
        std::cerr << "[" << rank << "] "
                  << "Error: cudaMemcpy (device to host) for result failed: " << cudaStat << std::endl;
        cudaFree(d_bVec);
        cudaFree(d_result);
        cudaFree(d_start_indices);
        return EXIT_FAILURE;
